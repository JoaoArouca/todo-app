<div class="container collumn tasks-group" wire:poll.5000ms>
    <div class="row">
        @if (count($tasks) > 0)
            @foreach ($tasks as $task)
                <div class="card task">
                    <div class="card-body">
                        @if ($task['is_editing'])
                            <input id="task_{{ $task['id'] }}_content" type="text" wire:model.defer="updateContent"
                                class="form-control">
                        @else
                    ...
                    <div class="card-footer">
                        <form wire:submit.prevent {{-- wire:dirty --}} wire:target="updateTask({{ $task['id'] }})"
                            ...
                            @csrf
                            @if ($task['is_editing'])
                                <button wire:click="updateContent({{ $task['id'] }})" class="btn btn-sm btn-success">
                                    Salvar
                                </button>
